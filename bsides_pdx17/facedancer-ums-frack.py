...
class FrackImage(FAT32DiskImage):
    ...
    def handle_fat_read(self, address):
        ...
        # HACK: If we're significantly past the valid FAT, generate the valid FAT again.
        if offset_into_fat > 2000:
            offset_into_fat -= 2000
            apply_hack_offset = True
            print("Applying FRACK to first subsequent sector!")
        ...
        if address == self.FAT_START and not apply_hack_offset:

            hack_offset= 0

            if self.sector_read_counts[address] > 2:
                print("Applying FRACK to first sector!")
                hack_offset = 2000
            ...
            response += self._generate_cluster_chain(first_cluster_in_fat_sector + 3, fw_clusters_in_this_sector, hack_offset)
        ...
        # ... otherwise, just generate the FAT entries for our firmware.
            ...
            hack_offset = 2000 if apply_hack_offset else 0

            fw_clusters_in_this_sector = min(clusters_per_fat_sector, clusters_past_this_sector)
            response = self._generate_cluster_chain(first_cluster_in_fat_sector, fw_clusters_in_this_sector, hack_offset)
    ...
    def _generate_cluster_chain(self, start_address, count=128, hack_offset=0):
        """
...
i = FrackImage(sys.argv[1], sys.argv[2], verbose=1)
